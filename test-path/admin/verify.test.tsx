...
jest.mock('next/navigation', () => ({
    ...
    useParams: () => ({ category: 'Verifikasi Inovator' }), // Sesuai dengan switch case di component
}));
...
describe('Pengujian Komponen Asli - Admin Verification Page', () => {
    ...
    test('Path 2: Memfilter data saat memilih status dari menu', async () => {
        ...
        // Buka menu filter
        const filterButton = screen.getByText(/Semua Status|Status|Semua/i);
        fireEvent.click(filterButton);

        // Pilih "Menunggu" dari menu
        const pendingOption = await screen.findByText('Menunggu');
