import { updateInnovator, createInnovator } from "Services/innovatorServices";
...
describe("Innovator Profile Basis Path Testing", () => {
    ...
    test("Path 1: Should update existing innovator profile", async () => {
        ...
        if (status !== "") {
            await updateInnovator("uid-123", { namaInovator: "Updated" });
    ...
    test("Path 2: Should create new innovator profile", async () => {
        ...
        if (status === "") {
            await createInnovator("uid-456", { namaInovator: "New" } as any);
            toast({ status: "success" });
        }
    ...
    test("Path 3: Should handle API errors in profile creation", async () => {
        ...
        try {
            await createInnovator("uid-789", { namaInovator: "Fail" } as any);
        } catch (e) {
            toast({ status: "error" });
        }
